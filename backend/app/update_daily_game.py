...
@tracer.capture_method
def generate_daily_game(emoji_count: int = DEFAULT_EMOJI_COUNT, 
                       answer_count: int = DEFAULT_ANSWER_COUNT) -> Dict[str, Any]:
    """
...
def upload_to_s3(s3_client, bucket_name: str, key: str, data: Any, content_type: str = 'application/json') -> bool:
    ...
    try:
        ...
        metrics.add_metric(name="S3Upload", unit="Count", value=1)
        return True
        
    except (ClientError, BotoCoreError) as e:
        logger.error(f"AWS error uploading {key} to {bucket_name}: {e}")
        metrics.add_metric(name="S3UploadError", unit="Count", value=1)
        return False
    except Exception as e:
...
def upload_file_to_s3(s3_client, bucket_name: str, file_path: str, key: str, content_type: str = 'font/ttf') -> bool:
    ...
    try:
        ...
        metrics.add_metric(name="S3FileUpload", unit="Count", value=1)
        return True
        
    except (ClientError, BotoCoreError) as e:
        logger.error(f"AWS error uploading file {file_path} to {bucket_name}: {e}")
        metrics.add_metric(name="S3FileUploadError", unit="Count", value=1)
        return False
    except Exception as e:
...
def invalidate_cloudfront_cache(distribution_id: str, paths: List[str]) -> bool:
    ...
    try:
        ...
        metrics.add_metric(name="CloudFrontInvalidation", unit="Count", value=1)
        return True
        
    except (ClientError, BotoCoreError) as e:
        logger.error(f"AWS error creating CloudFront invalidation: {e}")
        metrics.add_metric(name="CloudFrontInvalidationError", unit="Count", value=1)
        return False
    except Exception as e:
...
def lambda_handler(event: Dict[str, Any], context: LambdaContext) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            metrics.add_metric(name="FontGeneration", unit="Count", value=1)
        except Exception as e:
            logger.error(f"Error generating font subset: {e}")
            metrics.add_metric(name="FontGenerationError", unit="Count", value=1)
        ...
        try:
            ...
            metrics.add_metric(name="S3ClientError", unit="Count", value=1)
            return {
                'statusCode': 500,
        ...
        if not frontend_bucket:
            ...
            metrics.add_metric(name="ConfigurationError", unit="Count", value=1)
            return {
                'statusCode': 500,
        ...
        # Log success metrics
        metrics.add_metric(name="DailyGameUpdate", unit="Count", value=1)
        metrics.add_metric(name="EmojisGenerated", unit="Count", value=len(new_game_data['emojis']))
